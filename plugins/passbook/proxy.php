...
$valid_requests = array(
	'pass.salon-sante-nature.fr', 'pass.salon-sante-nature.dev',
);
...
if (CSAJAX_FILTERS) {
	...
	if (CSAJAX_FILTER_DOMAIN) {
		if (!in_array($parsed['host'], $valid_requests)) {
			csajax_debug_message('Invalid domain - ' . $parsed['host'] . ' does not included in valid requests #1');
			exit;
		}
	} else {
		...
		if (!in_array($check_url, $valid_requests)) {
			csajax_debug_message('Invalid domain - ' . $request_url . ' does not included in valid requests #2');
			exit;
		}
	}
}
